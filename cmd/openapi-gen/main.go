...
func configVars() *seq {
	vars := []configVar{
		...
		{Name: "WS_JITTER_BUFFER_MS", Default: "0", Description: "WebSocket ingress playout lead in ms (0 = disabled passthrough). Applies to every websocket leg and room WS participant. Use 40-60 when an external agent paces PCM against the mixer clock."},
		{Name: "WS_JITTER_BUFFER_MAX_MS", Default: "300", Description: "Maximum depth of the WS ingress buffer in ms when the jitter buffer is enabled."},
		{Name: "COMFORT_NOISE_ENABLED", Default: "true", Description: "Inject low-level comfort noise (~-75 dBFS) into silent mixer frames. Set false for cleaner telephony / recording A/B against MediaBridge."},
